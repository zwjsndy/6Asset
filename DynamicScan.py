...
def Start():
    ...
    parse.add_argument('--output',type=str, default=".",help='output directory name')
    parse.add_argument('--budget',type=int, default=400000, help='the upperbound of scan times')
    parse.add_argument('--IPv6',type=str, default="84.32.45.15", help='local IPv6 address')
    ...
    parse.add_argument('--batch_size', type=int, default=400000, help='the parameter for each epoch')
    parse.add_argument('--num_node', type=int, default=500, help='the parameter for each epoch')
    parse.add_argument('--ref', type=int, default=1,help='unchangeable')
    parse.add_argument('--random', type=int, default=0,help='unchangeable')
    parse.add_argument('--para', type=float, default=1,help='the parameter for "para"')
    ...
    logging.basicConfig(filename='my_{},alpha={},num_node={},batch={}.log,ref={},random={},para={}'.format(args.budget, args.alpha, args.num_node, args.batch_size,args.ref,args.random,args.para), level=logging.INFO)
    print("ipv6 address to sec beginning")
    print(args.input)
    V = InputAddrs(input=args.input, reflection=args.ref ,delta=args.delta,ran=args.random)
    print("ipv6 address to sec over")
    print("SpaceTreeGen beginning")
    root = SpaceTreeGen(V, delta=args.delta, beta=args.beta,para=args.para)
    print("SpaceTreeGen over")
    ScanPre(root,args.para)
    # OutputSpaceTree(root,V)
...
if __name__ == '__main__':
    ...
    logging.info('ALL_Hit rate: {}    ALL_time: {}'.format(hit_rate, time2-time1))
