...
def SpaceTreeGen(IPS, delta=16,beta=16,para=1):
    '''
    空间树生成
    ...
    '''
    root=TreeNode(IPS)
    DHC(root,beta,delta,para)

    return root
    
def DHC(node,beta,delta,para):
    '''
    层次聚类算法
    ...
    best_position=node.get_splitP(delta,para)
    if best_position==-1:
        return
